...
describe("workflow execution engine", () => {
  ...
  it("does not accept approval before delivery", () => {
    const run = advanceRun(createRun(), "buyer_approved");
    expect(run.approvals.buyer).toBe(false);
    expect(run.nodeRuns.payment).toBe("blocked");
  });

  it("keeps payment blocked with only one approval", () => {
    const delivered = advanceRun(createRun(), "delivery_received");
    const buyerApproved = advanceRun(delivered, "buyer_approved");
  ...
  it("settles and creates proof only after both approvals", () => {
    let run = advanceRun(createRun(), "delivery_received");
    run = advanceRun(run, "buyer_approved");
    run = advanceRun(run, "logistics_approved");
