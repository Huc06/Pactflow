...
function ProofCard({ proof }: { proof: NonNullable<WorkflowRun["proof"]> }) {
  return <section className="proof-card"><div className="proof-success"><span><Check size={22} /></span><div><small>VERIFIED ON SOLANA</small><h2>Execution proof created</h2><p>This cryptographic commitment covers the workflow, approvals, and settlement result without exposing private business data.</p></div></div><div className="proof-data"><div><small>PROOF HASH</small><code>{proof.proofHash}</code></div><div><small>TRANSACTION SIGNATURE · DEMO ADAPTER</small><code>{proof.signature}</code></div><div className="proof-meta"><span><small>NETWORK</small><b>{proof.network}</b></span><span><small>WORKFLOW VERSION</small><b>v1</b></span><span><small>APPROVALS</small><b>2 of 2</b></span></div><button className="secondary-button" disabled>Explorer available with live adapter <ExternalLink size={14} /></button></div></section>;
}
